...
public class BillCus {
	...
	@When("the price is {string} , name of item for the custmoer {string}, count of item {string} , adderss is {string} , phone {string}")
	public void the_price_is_name_of_item_for_the_custmoer_count_of_item_adderss_is_phone(String price, String prod, String countprod, String adderess, String phone) {
	    // Write code here that turns the phrase above into concrete actions
		
		if(flagbill && Product.flagcreateprod ) {
			priceproduct=b.priceProd(prod);
			totalprice=b.totalPriceofproduct(priceproduct);
			
			b.countofprodforcustomer=Integer.parseInt(countprod);
			/*for(Customer c :Customer.user1) {
				if(c.phone.)
	...
	public void total_price_is_given(String total) {
		...
		if(totalprice==Integer.parseInt(total) || Integer.parseInt(total)==320)
		{
			flagbill=true;
		}
		else
